...
def message_handle(update,bot:ObigramClient):
    ...
    if user:
        ...
        if ACCES_FREE:
            ...
            user.create_config()
        elif username == config.TG_USER:
            user = User(tg_id=str(message.sender.id),username=username)
            user.create_config()
        elif username == config.TG_USER1:
            user = User(tg_id=str(message.sender.id),username=username)
            user.create_config()
        elif username == config.TG_USER2:
            user = User(tg_id=str(message.sender.id),username=username)
            user.create_config()
        elif username == config.TG_USER3:
            user = User(tg_id=str(message.sender.id),username=username)
            user.create_config()
        else:
            access = False
...
if __name__ == '__main__':
    ...
    bot.send_message(5416296262,'___Iniciado___')
